...
import { getReceiptDataAction } from '@/app/actions';
...
async function ReceiptContent({
  ...
  const { order, items } = await getReceiptDataAction(orderId);

  // Compute exact total from actual database line items
    ...
    <div className="min-h-screen bg-slate-50 pb-20">
      <header className="bg-white border-b border-slate-200 sticky top-0 z-10 px-4 py-3 flex items-center justify-between print:hidden">
        <div className="flex items-center gap-3">
          <Link href={`/checkout?order=${orderId}&table=${tableId}&session=${sessionId}`} className="text-slate-500 hover:text-slate-800">
            <ArrowLeft className="w-5 h-5" />
          </Link>
          <h1 className="text-lg font-bold text-slate-900 leading-tight">Payment Receipt</h1>
        </div>
        <button
          type="button"
      ...
      <main className="max-w-md mx-auto p-4 space-y-4">
        <div className="restaurant-card p-6 bg-white space-y-6 shadow-sm rounded-2xl border border-slate-100">
          <div className="text-center space-y-2 border-b border-slate-100 pb-5">
            ...
            <h2 className="text-xl font-extrabold text-slate-900">Ocean Basket • Ikeja</h2>
            <p className="text-xs text-slate-500">Table {tableId} • Session {sessionId}</p>
          ...
          <div className="space-y-3">
            <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider">Order Items Summary</h3>
            <div className="space-y-2">
              {items.length === 0 ? (
                <p className="text-xs text-slate-400 text-center py-2">No items recorded for this receipt.</p>
              ) : (
                items.map((item: any) => (
          ...
          <div className="border-t border-slate-100 pt-4 space-y-2 text-sm">
            <div className="flex justify-between text-slate-600">
              ...
              <span className="font-semibold text-emerald-600">Confirmed (Database Logged)</span>
            </div>
          ...
          <div className="pt-2">
            ...
            >
              ...
              <span>Return to Home / New Session</span>
            </Link>
          </div>
        </div>
      </main>
