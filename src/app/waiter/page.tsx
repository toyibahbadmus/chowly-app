    ...
    <div className="min-h-screen bg-slate-50 pb-20">
      ...
      <main className="max-w-4xl mx-auto p-4 sm:p-6 space-y-6">
        <div className="flex items-center justify-between">
          <h2 className="text-xl font-extrabold text-slate-900 tracking-tight">Active Floor Orders (DB Persistent)</h2>
          <span className="text-xs bg-amber-100 text-amber-800 font-bold px-3 py-1 rounded-full">
            {incomingOrders.length} Total Orders Recorded
          </span>
        </div>

        {incomingOrders.length === 0 ? (
          <div className="restaurant-card p-12 bg-white text-center space-y-3">
            <p className="text-slate-500 text-sm">No orders currently found in the database.</p>
            <p className="text-xs text-slate-400">Place an order from the customer menu to see it appear here instantly!</p>
          </div>
        ) : (
          <div className="space-y-4">
            {incomingOrders.map((order: any) => (
              <div key={order.order_id} className="restaurant-card p-6 bg-white space-y-5">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between pb-4 border-b border-slate-100 gap-2">
                  <div className="space-y-1">
                    ...
                    <p className="text-xs text-slate-500">
                      Assigned Waiter: <strong className="text-slate-700">{order.waiter_name || 'David'}</strong> • Session: {order.session_id} • Time: {order.order_time}
                    </p>
                  </div>
